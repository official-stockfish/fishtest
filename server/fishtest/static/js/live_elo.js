...
function set_gauges(LLR, a, b, LOS, elo, ci_lower, ci_upper) {
    ...
    const LOS_chart_data = google.visualization.arrayToDataTable([["Label", "Value"], ["LOS", Math.round(1000 * LOS) / 10], ]);
    ...
    const LLR_chart_data = google.visualization.arrayToDataTable([["Label", "Value"], ["LLR", Math.round(100 * LLR) / 100], ]);
    ...
    const ELO_chart_data = google.visualization.arrayToDataTable([["Label", "Value"], ["Elo", set_gauges.last_elo], ]);
...
function display_data(items) {
    ...
    if (// Only notify if the test has a state (accepted or rejected)
    items.args.sprt.state && // Only notify if there is already text in LLR (the test wasn't just opened)
    document.getElementById("LLR").textContent !== "" && supportsNotifications() && Notification.permission === "granted") {
