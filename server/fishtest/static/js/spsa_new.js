...
function spsa_compute(spsa_setup) {
    ...
    for (let i = 0; i < s.num_params; i++) {
        ...
        s.params[i].r = r;
        const H_diag = (-2 * s.params[i].elo) / Math.pow((s.params[i].max - s.params[i].min) / 2, 2);
...
function tc_to_seconds(tc) {
  ...
  */
    let inc = 0;
    let moves = 68;
    /* Fishtest average LTC game duration. */
    let time;
    let chunks = tc.split("+");
    if (chunks.length > 2) {
        return null;
...
function draw_ratio(tc) {
  ...
  */
    ...
    return tc_seconds !== null ? logistic(slope * Math.log(tc_seconds) + intercept) : null;
}
