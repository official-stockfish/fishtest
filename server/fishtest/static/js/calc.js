...
google.charts.setOnLoadCallback(function() {
    ...
    mouse_screen.addEventListener("click", function(e) {
        e.stopPropagation();
    }, true);
    mouse_screen.addEventListener("mouseover", function(e) {
        e.stopPropagation();
    }, true);
...
function draw_charts() {
    ...
    for (let i = elo_start * N; i <= elo_end * N; i += 1) {
        ...
        data_pass.push([elo, null, {
            v: c[0],
            f: (c[0] * 100).toFixed(1) + "%"
        }]);
        data_expected.push([elo, null, {
            v: c[1],
            f: (c[1] / 1000).toFixed(1) + "K"
        }, ]);
        for (const elo_ of specials) {
            if (elo < elo_ && elo_next >= elo_) {
                anchors.push(elo_);
                const c_ = sprt.characteristics(elo_);
                data_pass.push([elo_, elo_, {
                    v: c_[0],
                    f: (c_[0] * 100).toFixed(1) + "%"
                }, ]);
                data_expected.push([elo_, elo_, {
                    v: c_[1],
                    f: (c_[1] / 1000).toFixed(1) + "K"
                }, ]);
            }
        }
    }
    ...
    const gridlines_style = {
        color: "#666"
    };
    const minor_gridlines_style = {
        color: "#aaa"
    };
