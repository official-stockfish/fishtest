...
<script>
  ...
  async function handleToggleUsers() {
    ...
    const tableHandlers =
      Array.from(document.getElementById("users-table-handler").querySelectorAll(".dropdown-item"));
    tableHandlers.forEach((tableHandler) => {
      tableHandler.addEventListener("click", (e) => {
        const tableBodies =
          Array.from(document.getElementById("users-table").querySelectorAll("tbody"));
