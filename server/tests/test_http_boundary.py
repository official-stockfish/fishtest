...
class TestHttpBoundary(unittest.TestCase):
    ...
    def test_is_hx_request_rejects_full_page_requests(self):
        ...
        # A back navigation carries no HX-Request.
        ...
        # A request that states no scope is a fragment request.
    ...
    def test_oob_attributes_are_never_nested_inside_a_template(self):
        # htmx does not find out-of-band elements inside <template> content.
    ...
    def test_poll_conditions_are_attached_to_the_every_token(self):
        # htmx reads a [condition] off the leading trigger token only.
        # Details: docs/5-templates.md (rule 17, "Polling trigger policy").
        templates_dir = Path(__file__).resolve().parents[1] / "fishtest" / "templates"
        ...
        self.assertEqual(violations, [], "\n".join(sorted(set(violations))))
        # Fail rather than pass by scanning nothing.
    ...
    def test_history_restore_request_gets_a_full_page(self):
        # A back navigation refetches the pushed URL and must get a whole page,
        # with or without an HX-Request header alongside.
        app = self._build_app(include_views=True)
        client = self.TestClient(app)
    ...
    def test_live_elo_update_terminal_sprt_removes_poller(self):
        ...
        # A terminal SPRT state deletes the poll driver.
        self.assertEqual(response.status_code, 200)
